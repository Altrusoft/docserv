 ...
 * 
 * Integration tests actually starting an instance of the server and then make HTTP requests and
 * verifies the output.
 * 
 */
public class IntegrationTest {
	...
	private static final String DOCSERVER_BASE_URL = "http://localhost:" + DOCSERVER_PORT;

	// @Test
	// public void goingToRootShouldShowPageWithListsOfRoutes() {
	// running(testServer(DOCSERVER_PORT, fakeApplication()), HTMLUNIT, new Callback<TestBrowser>()
	// {
	// @Override
	...
	// Assert.assertTrue(pageSource.contains("Action not found"));
	...
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType("invoiceOdt",
	// MimeType.ODS.getValue(), "odt/expected_odt_invoice.odt");
	// }

	// @Test
	// public void
	// postOnODTTemplateWithoutClassURLWithJsonParamsAndAcceptingODT_ShouldGenerateFeededODT() {
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType("invoiceNoClassOdt",
	// MimeType.ODS.getValue(), "odt/expected_odt_invoice_no_class.odt");
	// }

	// @Test
	// public void postOnODTTemplateURLWithJsonParamsAndAcceptingPDF_ShouldGenerateFeededPDF() {
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType("invoiceOdt",
	// MimeType.PDF.getValue(), "odt/expected_odt_invoice.pdf");
	// }

	// @Test
	// public void postOnODSTemplateURLWithJsonParamsAndAcceptingODS_ShouldGenerateFeededODS() {
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType("invoiceOds",
	// MimeType.ODS.getValue(), "ods/expected_ods_invoice.ods");
	// }

	// @Test
	// public void postOnODSTemplateURLWithJsonParamsAndAcceptingPDF_ShouldGenerateFeededPDF() {
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType("invoiceOds",
	// MimeType.PDF.getValue(), "ods/expected_ods_invoice.pdf");
	// }
	...
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType("invoiceOds",
	// MimeType.XLS.getValue(), "ods/expected_ods_invoice.xls");
	// }

	// private void
	// postOnTemplateURLWithJsonParams_ShouldGenerateFeededDocumentWithAcceptedMimeType(
	// final String templateName, final String requestedMimeType, final String
	...
	// try {
	// File paramFile = application.getFile(jsonParamFilePath);
	// jsonParams = FileUtils.readFileToString(paramFile);
	// } catch (IOException e) {
	// fail("Can't open file json param file \"" + jsonParamFilePath + "\": (" + e.toString() +
	// ")");
	// }
	// WSRequest url = WS.url(DOCSERVER_BASE_URL + "/document/" + templateName);
	...
	// assertEquals(200, response.get(1L, TimeUnit.MINUTES).getStatus());
	//
	// File expectedFile = application.getFile("test/" + fileNameOfExpectedDocument);
	// assertResponseEqualsFileContent(expectedFile, response, true);
	// }
	//
	// });
	// }
	...
	// Application application = Play.application();
	//
	// WSRequest url = WS.url(DOCSERVER_BASE_URL + "/assets/templates/Invoice/template.odt");
	...
	// assertEquals(200, response.get(1L, TimeUnit.MINUTES).getStatus());
	//
	// File expectedFile = application.getFile("public/templates/Invoice/template.odt");
	// assertResponseEqualsFileContent(expectedFile, response, false);
	// }
	// });
	// }

	void assertResponseEqualsFileContent(File expectedFile, Promise<WSResponse> response, boolean onlyVerifySize) {
		...
		try {
			...
			actualInputStream = response.get(5000L).getBodyAsStream();
			...
			if (expectedFile.getName().endsWith(".pdf")) {
				...
				assertEquals("Not same text on first page", expectedPageOne, actualPageOne);
			} else {
				if (onlyVerifySize) {
					...
					if (!hasGeneratedFileCorrectSize) {
						fail("Actual file is not same size expected (exp size=" +
							new String(expectedByteArray).length() + " act size=" +
			...
			fail("Can't compare generated file with expected file: " + e.getMessage());
		} finally {
	...
	private String getNonWhiteSpacesFromPDF(byte[] pdfByteArray) throws IOException {
		String nonWhiteSpace;

		PdfReader pdfReader = null;
		try {
			pdfReader = new PdfReader(pdfByteArray);
			nonWhiteSpace = StringUtils.deleteWhitespace(PdfTextExtractor.getTextFromPage(pdfReader, 1));
